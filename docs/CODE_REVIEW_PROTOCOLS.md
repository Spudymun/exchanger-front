...
- **Размер функций** ≤50 строк (CODE_STYLE_GUIDE.md)
...
2. **Качественная проверка** - размер, сложность, читаемость
...
3. **Анализ размера функций:** ESLint rule `max-lines-per-function`
4. **Проверка зависимостей:** импорты только из constants/types
5. **DRY принцип:** отсутствие дублирования логики между утилитами
6. **Строгое использование констант:** все магические числа/строки заменены на константы из уровня 1
...
- [ ] Размер роутера ≤150 строк (или разбивка на подроутеры)
...
2. **Превышение размера роутера:**

   ```typescript
   // ❌ Роутер на 300+ строк
   export const exchangeRouter = createTRPCRouter({
     // 20+ процедур в одном роутере
   });
   ```
...
2. **Проверка размера роутеров:** grep -c "export const.\*=" каждого роутера
3. **Анализ валидации:** все процедуры с input должны иметь схему
4. **Проверка ролевой модели:** соответствие архитектуре из ROLES_ARCHITECTURE.md
5. **DRY принцип:** отсутствие дублирования логики между роутерами
6. **Межуровневые зависимости:** правильное использование констант из уровня 1 и утилит из уровня 2
...
3. **Анализ размера:** stores не более 200 строк
...
2. **Проверка размера:** wc -l для каждого компонента
3. **Анализ пропсов:** количество параметров в интерфейсах
...
- Превышение размера функций (>50 строк)
...
**Версия:** 1.0  
**Дата создания:** 4 июля 2025  
**Основа:** ai-agent-rules.yml + CODE_STYLE_GUIDE.md + senior best practices
