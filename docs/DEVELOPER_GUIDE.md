...
exchanger-front/
├── apps/                          # Приложения
│   ├── web/                       # Основное приложение (3000)
│   ├── admin-panel/               # Админ панель (3002)
│   └── docs/                      # Документация (3001)
├── packages/                      # Переиспользуемые пакеты
│   ├── ui/                        # UI-библиотека
│   ├── design-tokens/             # Дизайн токены
│   ├── hooks/                     # React хуки + состояние
...
│   │   │   ├── input.tsx
│   │   │   └── ...
│   │   ├── data-table.tsx         # Составные компоненты
│   │   ├── theme-toggle.tsx       # Функциональные компоненты
│   │   └── tree-view.tsx
...
│   │   ├── trading-store.ts       # Бизнес логика
│   │   └── ui-store.ts            # UI состояние
│   ├── use-local-storage.ts       # Утилитарные хуки
│   └── index.ts
└── package.json
```
...
│   ├── client.ts                  # Устаревший HTTP client
│   └── index.ts
└── package.json
```
