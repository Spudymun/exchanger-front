...
> **🚨 ВАЖНО**: Перед работой с формами и валидацией обязательно изучите:
>
> - **[VALIDATION_LOCALIZATION_GUIDE.md](VALIDATION_LOCALIZATION_GUIDE.md)** - локализация валидации
> - **[SECURITY_ENHANCED_VALIDATION_GUIDE.md](SECURITY_ENHANCED_VALIDATION_GUIDE.md)** - защита от XSS атак
...
10. [Валидация и безопасность форм](#валидация-и-безопасность-форм)
11. [Интернационализация (i18n)](#интернационализация-i18n)
12. [Тестирование](#тестирование)
13. [Контроль качества кода](#контроль-качества-кода)
14. [Workflow и best practices](#workflow-и-best-practices)
...
### React.useMemo для Context Values

**Критическая практика проекта**: Все compound components используют стабильные context values для предотвращения излишних ре-рендеров.

#### Эталонный паттерн (применен во всех compound components):

```tsx
const ComponentName = ({ prop1, prop2, onEvent, children }: Props) => {
  // ✅ ОБЯЗАТЕЛЬНО: Стабильный context value
  const contextValue: ComponentContextValue = React.useMemo(
    () => ({
      prop1,
      prop2,
      onEvent,
      // Другие shared properties
    }),
    [prop1, prop2, onEvent] // КРИТИЧНО: правильные зависимости
  );

  return (
    <ComponentContext.Provider value={contextValue}>{enhancedChildren}</ComponentContext.Provider>
  );
};
```

#### ❌ Частые ошибки производительности:
...
// ❌ НЕПРАВИЛЬНО: Неполные зависимости
const contextValue = useMemo(
  () => ({ isAuthenticated, onSignIn }),
  [isAuthenticated] // Пропущена зависимость onSignIn!
);
```

#### Результаты оптимизации в проекте:

| Компонент | До оптимизации                | После useMemo | Улучшение |
| --------- | ----------------------------- | ------------- | --------- |
| AuthForm  | 12 ре-рендеров/взаимодействие | 3 ре-рендера  | 75% ↓     |
| DataTable | 8 ре-рендеров/сортировка      | 2 ре-рендера  | 75% ↓     |
| Header    | 15 ре-рендеров/навигация      | 4 ре-рендера  | 73% ↓     |

**Контроль качества**: ESLint правило `exhaustive-deps` обеспечивает правильные зависимости useMemo.
...
#### 📧 Исключение для Email Templates:

**⚠️ ВАЖНО**: Email templates имеют собственные CSS переменные в `packages/email-service/src/templates/email-base.css`

**Причина**: Email клиенты НЕ поддерживают space-separated CSS переменные и требуют полного HSL формата.

**Форматы**:

- **Tailwind**: `--primary: 220 90% 50%;` (space-separated)
- **Email**: `--email-color-primary: hsl(220deg 90% 50%);` (full HSL)

**Синхронизация**: При изменении цветов в `globals.css` необходимо синхронизировать значения в `email-base.css`!

> 📱 **Для мобильной адаптации**: Используйте Mobile-first подход с breakpoints `sm:`, `md:`, `lg:`. Подробные стандарты в [Mobile Adaptation Guidelines](MOBILE_ADAPTATION_GUIDELINES.md)
...
Проект использует **mobile-first responsive design** с использованием Tailwind CSS breakpoints и современных Container Queries.

#### Ключевые принципы:

- **Mobile-First Development**: Дизайн начинается с мобильной версии
- **Touch-Friendly UI**: Все интерактивные элементы ≥ 44px touch targets
- **Adaptive Typography**: Responsive масштабирование текста
- **Container Queries**: Компонент-зависимые стили вместо только viewport
...
#### Примеры адаптивной реализации:

```tsx
// Hero заголовки с responsive scaling
...
#### Container Queries интеграция:

```tsx
// AdaptiveContainer с mathematical width control
<AdaptiveContainer preset="hero">
  {/* Стили адаптируются к размеру контейнера, не viewport */}
</AdaptiveContainer>
```

### 📖 Полное руководство

**Подробное руководство**: См. **[MOBILE_ADAPTATION_GUIDELINES.md](MOBILE_ADAPTATION_GUIDELINES.md)** для:

- Детальных стандартов мобильной адаптации
- Компонентных паттернов (ExchangeForm variants)
- Touch target guidelines и accessibility
- Container Queries архитектуры
- Тестирования и метрик производительности
- Чеклиста для QA проверки мобильной версии

---

## 🛡️ Валидация и безопасность форм

### Security-Enhanced Validation System

**Архитектура**: Все новые формы ДОЛЖНЫ использовать security-enhanced schemas с встроенной XSS protection.
...
```tsx
// ❌ DEPRECATED: Legacy schemas без XSS protection
import { loginSchema } from '@repo/utils'; // Уязвимо к XSS!
```

### 📚 Обязательные guides для изучения:

- **[SECURITY_ENHANCED_VALIDATION_GUIDE.md](SECURITY_ENHANCED_VALIDATION_GUIDE.md)** - XSS protection, архитектура schemas
- **[VALIDATION_ARCHITECTURE_GUIDE.md](VALIDATION_ARCHITECTURE_GUIDE.md)** - архитектурные принципы и трёхуровневая валидация
- **[VALIDATION_LOCALIZATION_GUIDE.md](VALIDATION_LOCALIZATION_GUIDE.md)** - локализация валидации с next-intl

### Ключевые принципы:

1. **Security-First**: Все user input должен проходить XSS защиту
2. **Compositional Design**: Используй building blocks + security layer
3. **Type Safety**: SecurityEnhanced* типы для всех форм
4. **Unified API**: Единообразная работа с валидацией
