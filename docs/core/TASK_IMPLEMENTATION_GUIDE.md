...
| Стилизация         | **Tailwind CSS + Centralized CSS**       | Всегда                       |
| CSS Variables      | **packages/tailwind-preset/globals.css** | Единственный источник истины |
| Темизация          | **Автоматическая light/dark поддержка**  | Встроено в систему           |

**📋 Чек-лист UI (обновлен 2025):**
...
- [ ] ✅ **CSS переменные централизованы** → `packages/tailwind-preset/globals.css`
- [ ] ✅ **@import настроен** → `@import '@repo/tailwind-preset/globals.css'` в каждом приложении
- [ ] Используете централизованные переменные? → `bg-card`, `text-foreground`
- [ ] Проверить темизацию? → Тестировать в light/dark режимах

**🎨 Новая CSS архитектура - Быстрые паттерны:**

```typescript
// ✅ Правильное использование централизованных переменных
import { cn } from '@repo/ui';

// Стандартная карточка обмена
<div className={cn(
...
// ❌ НЕ ОПРЕДЕЛЯЙТЕ CSS переменные в отдельных файлах
// ❌ НЕ ДУБЛИРУЙТЕ :root { --background: ... }
// ✅ ИСПОЛЬЗУЙТЕ только централизованные переменные
```

**🚨 Критические правила новой архитектуры:**

1. **Единый источник**: `packages/tailwind-preset/globals.css` - ЕДИНСТВЕННОЕ место для CSS переменных
2. **Обязательный @import**: `@import '@repo/tailwind-preset/globals.css'` в КАЖДОМ приложении
3. **Запрет дублирования**: НЕ определяйте CSS переменные нигде больше
4. **Семантические классы**: Используйте `bg-card`, `text-foreground` вместо прямых значений
...
| ------------ | ------------------ | -------------------------------------------- |
| Новые тексты | Добавить переводы  | `apps/[app]/messages/[locale]/[module].json` |
...
**🚨 ВАЖНО:** Используйте [DEVELOPER_GUIDE.md](DEVELOPER_GUIDE.md) и [I18N_TROUBLESHOOTING.md](../troubleshooting/I18N_TROUBLESHOOTING.md) для правильной настройки i18n!
...
📦 Новые переводы
└── apps/[app]/messages/[locale]/[module].json
...
1. **Добавить переводы во все locale файлы (модульно по доменам)**
2. **Использовать в компонентах**

```json
// apps/web/messages/en/common-ui.json
{
  "tasks": {
    "title": "Tasks",
    "create": "Create Task",
